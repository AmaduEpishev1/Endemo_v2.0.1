"""
This module contains all classed used for the first stage of run.
"""
...
from endemo2.data_structures.prediction_methods import Timeseries, RigidTimeseries, IntervalForecast
...
from endemo2.run.preprocessing_utility import energy_carrier_to_energy_consumption
...
class IndustryPreprocessed:
    ...
    def __init__(self, country_name, input_manager: InputManager, pop_his: Timeseries, gdp_his: Timeseries):
        ...
        general_input = input_manager.general_input_path

        for product_name, product_input in input_manager.IND_input.dict_product_input.items():
...
class CountryPreprocessed:
    ...
    def __init__(self, country_name, input_manager: InputManager):
        ...
        if SectorIdentifier.IND in active_sectors:
        ...
        if SectorIdentifier.CTS in active_sectors:
            self.cts_pp = CtsPreprocessed(country_name, input_manager.general_input_path, input_manager.CTS_input,
                                          self.population_pp.population_historical_whole_country,
                                          self.nuts2_pp.population_historical_tree_root)
        # if SectorIdentifier.HOU in active_sectors:
        #     self.households_pp = HouseholdsPreprocessed(country_name, input_manager.general_input,
        #                                                 input_manager.hh_input,
        #                                                 self.population_pp.population_historical_whole_country)
        if SectorIdentifier.TRA in active_sectors:

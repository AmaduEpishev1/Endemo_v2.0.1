...
from endemo2.data_structures.enumerations import ForecastMethod, DemandDriver
...
def swap_and_zip_datapoints(datapoints: List[Datapoint]) -> List[Datapoint]:
    """
    Takes a list of Datapoints and swaps the `x` and `y` values for each Datapoint.

    :param datapoints: List of Datapoint objects.
    :return: List of Datapoint objects with swapped `x` and `y`.
    """
    return [Datapoint(dp.y, dp.x) for dp in datapoints]

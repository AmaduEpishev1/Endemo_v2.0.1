...
class Forecast:
    ...
    def _calc_coeff_hist(self,region):
        ...
        for index, row in historical_data.iterrows():
            ...
            demand_driver_data = self._map_filtered_demand_driver_data(demand_drivers, year_columns_list, region.region_name, #df mapped for historical data coef calculation
                                                                   data_origin="historical",forecast_method=forecast_method)
    ...
    def _map_filtered_demand_driver_data(self, demand_drivers, years, region_name, data_origin,forecast_method):
        ...
        for driver_name in demand_drivers:
            ...
            if region_data is not None and not region_data.empty:
                filtered_values = filter_values_by_year(region_data, years, driver_name) #df
                filtered_values.index = filtered_values.index.astype(int)
                aligned_DDr_df[driver_name] = filtered_values[driver_name]
            else:
                print(f"No data for demand driver '{driver_name}' in region '{region_name}'.")
                aligned_DDr_df[driver_name] = np.nan #if we put the data of non-existing DDr name in the database
        # Determine if we can tolerate NaN based on the forecast method
        if forecast_method == ForecastMethod.CONST_LAST:
            # Allow NaNs for methods that don't rely on demand driver data
            return aligned_DDr_df
        else:
            # Drop rows with NaN values for other forecast methods
...
def filter_values_by_year(region_data: pd.DataFrame, years: list, driver_name) -> pd.DataFrame:
    """
    ...
    :param driver_name: name of DDr (string) to filter the data by.
    :return: pd.DataFrame of values corresponding to the specified years.
    """
    ...
    filtered_data = region_data[['Region'] + matching_columns].set_index('Region')
    # Transpose to have years as the index
    filtered_data = filtered_data.T
    # Rename the column (e.g., "Region" to "DDr name")
    filtered_data.columns = [driver_name] #TODO check why the problem is here
